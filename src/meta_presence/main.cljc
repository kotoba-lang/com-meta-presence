(ns meta_presence.main
  "Kotodama WASM entrypoint for the Meta_presence clean-room actor (L4) — Clojure port.
  ...
  facts (`meta_presence.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "meta_presence")
(def tier "L4")
...
(def entity-specs
  [{:entity "Anchor"   :plural "anchors"   :id-prefix "metapres_anc"
    :fields [:sceneId :x :y :z :persistent]        :required [:x :y]
    :coerce {:x :float :y :float :z :float :persistent :bool} :refs {:sceneId "Scene"}}
   {:entity "Scene"    :plural "scenes"    :id-prefix "metapres_sce"
    :fields [:name :deviceId :meshRef]             :required [:name]
    :coerce {}                       :refs {:deviceId "Device"}}
   {:entity "Mesh"     :plural "meshes"    :id-prefix "metapres_mes"
    :fields [:sceneId :vertexCount :contentRef]   :required [:vertexCount]
    :coerce {:vertexCount :int}      :refs {:sceneId "Scene"}}
   {:entity "Session"  :plural "sessions"  :id-prefix "metapres_ses"
    :fields [:deviceId :appId :startedAt]         :required [:startedAt]
    :coerce {}                       :refs {:deviceId "Device"}}
   {:entity "Asset"    :plural "assets"    :id-prefix "metapres_ass"
    :fields [:sceneId :type :contentRef]          :required [:type]
    :coerce {}                       :refs {:sceneId "Scene"}}
   {:entity "Device"   :plural "devices"   :id-prefix "metapres_dev"
    :fields [:model :platform :trackingMode]      :required [:model :platform]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"meta_presence.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "meta_presence-compat" :tier tier :entities entities} 200])
...
(defn start! [] :meta_presence-compat/ready)
